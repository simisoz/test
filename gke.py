...
default_args = {
    'owner': 'me',
    'start_date': dt.datetime(2018, 4, 5),
    'retries': 1,
    'retry_delay': dt.timedelta(minutes=5),
}

dag = DAG(
    dag_id='gke', default_args=default_args,
...
t3 = BashOperator(
     task_id="destroy_spark", dag=dag, bash_command="sleep 30 && helm init --client-only && helm delete spark && helm delete --purge spark",
     executor_config={"KubernetesExecutor": {
         "image": "lachlanevenson/k8s-helm:latest"}}
 )
